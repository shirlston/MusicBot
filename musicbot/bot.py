...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None):
        ...
        self.http.user_agent += ' Project Helix/%s' % BOTVERSION
    ...
    def tfk_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if orig_msg.author.id in self.config.tfk_ids:
                # noinspection PyCallingNonCallable
                return await func(self, *args, **kwargs)
            else:
                raise exceptions.PermissionsError("only Syndicate Rider Server users can use this command", expire_in=30)

        wrapper.tfk_cmd = True
        return wrapper
            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        ...
                        'Use %sjoin to make Project Helix join your voice your voice channel' % self.config.command_prefix)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = 'Now Playing: **%s** in ***%s*** reqested by **%s**' % (
                 entry.title, player.voice_client.channel.name, author.name)
            else:
...
#######################################################################################################################
    ...
    async def cmd_thelp(self, command=None):
        """
        Usage:
            {command_prefix}help2 [command]
            a backup for the main thelp cmd
        """
    ...
    @owner_only
    async def cmd_joinserver(self, author, message, server_link=None):
        """
        Usage:
        ...
        try:
            if server_link:
                ...
                return Response("**%s** Use this to invite me to other servers" % author.name)
    ...
    async def cmd_remove(self, player, leftover_args, index):
        ...
        try:
            ...
            if index > playlist_size:
                if(playlist_size > 1):
                    reply_text = "There are only %s songs in the queue, queue something up with .play!"
                    reply_text %= (playlist_size)
                    return Response(reply_text, delete_after=30)
                elif(playlist_size == 1):
                    reply_text = "There is only %s song in the queue, queue something up with .play!"
                    reply_text %= (playlist_size)
                    return Response(reply_text, delete_after=30)
                else:
                    reply_text = "There aren't any songs in the queue, queue something up with .play!"
                    return Response(reply_text, delete_after=30)
            ...
            reply_text = "Must specify an index to remove in another word a number"
    ...
    async def cmd_diceroll(self, channel, author, leftover_args):
       ...
       an  example = .diceroll 1 d20
        ...
        if len(diceVals) == 2:
            ...
            return Response(":game_die: **%s** used **%s** to roll a **%d.**" % (author.name, diceInput, rollSum), delete_after=30)
        else:
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        busymsg = await self.safe_send_message(
            channel, "Enqueueing %s songs..." % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
        entries_added = 0
    ...
    async def cmd_stream(self, player, channel, author, permissions, song_url):
        ...
        return Response("**%s** - your stream will start soon" % author.name, delete_after=20)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        return Response("Oh well Sorry", delete_after=30)
    ...
    async def cmd_join(self, channel, server, author, voice_channel):
        ...
        if not author.voice_channel:
            ...
            await self.safe_send_message(channel, "**Connected! :wave::skin-tone-1:**")
    ...
    async def cmd_qclear(self, player, author):
        """
        Usage:
            {command_prefix}qclear
        Clears the queue.
        """

        player.playlist.clear()
        return Response('**%s** has cleared the queue' % author.name, delete_after=20)
    ...
    async def cmd_voteskip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if skips_remaining <= 0:
            ...
            return Response(
              'The vote to skip  **{}** has passed'.format(
                    player.current_entry.title,
                    ' Up Next!' if player.playlist.peek() else ''
                ),
                reply=True,
                delete_after=20
            )
            ...
            return Response(
                'Your skip for **{}** recognized but'
                '\n**{}** more {} required to vote to skip this song.'.format(
                    player.current_entry.title,
                    skips_remaining,
                    'vote' if skips_remaining == 1 else 'votes'
                ),
                reply=True,
                delete_after=20
            )
    ...
    async def cmd_forceskip(self, channel, author, user_mentions, player, message):
        ...
        Usage:
            ...
            This is used by admins and server staff to forceskip something that should not be playing
        """

        if player.is_stopped:
            raise exceptions.CommandError("There's Nothing Playing! Error: Can Not Skip", expire_in=20)

        if player.is_paused:
            raise exceptions.CommandError("Player is Paused Error: Can Not Skip", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("[Info:] Downloading Please Wait" % player.playlist.peek().title)

                elif player.playlist.peek().is_downloaded:
                    log.info('Downloading the next song, please wait')
                else:
                    await self.safe_send_message(channel, "Project Helix has stopped responding")
            else:
                await self.safe_send_message(channel, "Something odd is happening, do the restart command to fix this")
        ...
        if player.is_playing:
            return Response('The song has been forcibly skipped by **%s**' % author.name, delete_after=35)

    async def cmd_volume(self, message, author, player, new_volume=None):
        """
        Usage:
    ...
    async def cmd_restartall(self, author, channel):
        for s in self.servers:
    ...
    async def cmd_night(self, channel, message, author, user_mentions):
        """
        Use: {command_prefix}night
        Tells everyone good night for you
        """
        await self.safe_send_message(channel, "**%s** is going to bed everyone, be sure to say goodnight!" % author.name, expire_in=35)

    async def cmd_morning(self, channel, message, author, user_mentions):
        """
        Use: {command_prefix}morning
        Tells You good morning for you
        """
        await self.safe_send_message(channel, "Good Morning *%s*" % author.name, expire_in=35)
    ...
    async def cmd_purge(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('Erased {} message{}.'.format(len(deleted), 's' * bool(deleted)), delete_after=15)
        ...
        return Response('Erased {} message{}.'.format(deleted, 's' * bool(deleted)), delete_after=6)

    async def cmd_pldump(self, author, channel, song_url):
        """
        Usage:
        ...
        return Response("**%s** Here's your url dump" % author.name, delete_after=20)
    ...
    async def cmd_listids(self, server, author, leftover_args, cat='all'):
        ...
        return Response("**%s** the role list has been sent via direct message" % author.name, delete_after=20)
    ...
    async def cmd_perms(self, author, channel, server, permissions):
        ...
        return Response("**%s** your permissions are in direct messages" % author.name , delete_after=20)


    @owner_only
    async def cmd_setname(self, author, leftover_args, name):
        """
        Usage:
        ...
        return Response("**%s** new name set" % author.name, delete_after=20)

    async def cmd_setnick(self, server, channel, author, leftover_args, nick):
        """
        Usage:
        ...
        return Response("**%s** new nickname set" % author.name, delete_after=20)

    @owner_only
    async def cmd_setavatar(self, author, message, url=None):
        """
        Usage:
        ...
        return Response("**%s** set the avatar" % author.name, delete_after=20)

    async def cmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("**Disconnected** :wave::skin-tone-1:", delete_after=20)

    async def cmd_restart(self, channel):
        await self.safe_send_message(channel, "**Restarting** :wave::skin-tone-1:")
        await self.disconnect_all_voice_clients()
        raise exceptions.RestartSignal()
    @owner_only
    async def cmd_shutdown(self, channel):
        await self.safe_send_message(channel, "**Shuting Down** :wave::skin-tone-1:")
        await self.disconnect_all_voice_clients()
    ...
    async def on_server_join(self, server:discord.Server):
        ...
        if not self.user.bot:
            alertmsg = "<@{uid}> Hi I'm a Project Helix please mute me."
